defmodule Uniris.Bootstrap.Sync do
  ...
  @out_of_sync_date_threshold Application.compile_env(:uniris, [__MODULE__, :out_of_sync_date_threshold])

  @doc """
  Determines if network should be initialized
  """
  # ASK: why there is always 1 transaction?
  # ASK: should the list be empty on the initialization?
  @spec should_initialize_network?(list(Node.t()) | nil) :: boolean()
  def should_initialize_network?([]) do
    IO.puts "SHOULD INIT NETWORK ?: EMPTY"
    # count = TransactionChain.count_transactions_by_type(:node_shared_secrets) == 0
    count = TransactionChain.count_transactions_by_type(:node_shared_secrets) == 1
    IO.puts "TX COUNT: #{inspect TransactionChain.count_transactions_by_type(:node_shared_secrets)}"
    count
  end

  def should_initialize_network?([%Node{first_public_key: node_key} | _]) do
    IO.puts "SHOULD INIT NETWORK ?: #{inspect Base.encode16(node_key)}"
    IO.puts "SHOULD INIT NETWORK ?: #{inspect Base.encode16(Crypto.node_public_key(0))}"
    IO.puts "SHOULD INIT NETWORK ?: #{inspect node_key == Crypto.node_public_key(0)}"
    IO.puts "SHOULD INIT NETWORK ?: #{inspect TransactionChain.count_transactions_by_type(:node_shared_secrets)}"
    IO.puts "SHOULD INIT NETWORK ?: #{inspect TransactionChain.count_transactions_by_type(:node_shared_secrets) == 0}"
    node_key == Crypto.node_public_key(0) and TransactionChain.count_transactions_by_type(:node_shared_secrets) == 0
  end

  def should_initialize_network?(_) do
    IO.puts "SHOULD INIT NETWORK ?: OTHER"
    false
  end
  ...
  def require_update?(ip, port, last_sync_date) do
    ...
    case P2P.list_nodes() do
      [%Node{first_public_key: ^first_node_public_key}] -> false
      _ ->
        ...
        case P2P.get_node_info(first_node_public_key) do
          ...
          when ip != prev_ip or port != prev_port or diff_sync > @out_of_sync_date_threshold -> true
          _ -> false
        end
    end
  end
  ...
  def initialize_network(ip, port) do
    ...
    IO.puts "INITIALIZE NETWORK: #{inspect ip}:#{inspect port}"
    IO.puts "INITIALIZE NETWORK: #{inspect tx}"
  ...
  def get_closest_nodes_and_renew_seeds(bootstrapping_seeds, patch) do
    ...
    IO.puts "11111: #{inspect closest_nodes}"
    IO.puts "22222: #{inspect new_seeds}"
